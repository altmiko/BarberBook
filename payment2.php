    ...
    if (empty($services)) {
        ...
        try {
            ...
            foreach ($services as $item) {
                ...
                // Check if customer has a valid referral code and hasn't used it before
                $referralCheckQuery = "SELECT c.UsedReferralCode, 
                                     (SELECT COUNT(*) FROM Appointments a WHERE a.CustomerID = c.UserID) as appointment_count
                                     FROM Customers c 
                                     WHERE c.UserID = ?";
                $referralStmt = $conn->prepare($referralCheckQuery);
                if (!$referralStmt) {
                    throw new Exception('Database prepare error: ' . $conn->error);
                }
                $referralStmt->bind_param("i", $customerID);
                $referralStmt->execute();
                $referralResult = $referralStmt->get_result();
                $referralData = $referralResult->fetch_assoc();
                $referralStmt->close();

                // Apply 10% discount if customer has a referral code and this is their first appointment
                if ($referralData['UsedReferralCode'] && $referralData['appointment_count'] == 0) {
                    $item['price'] = $originalPrice * 0.9; // Apply 10% discount
...
<div class="payment-container">
    ...
    <div class="order-summary">
            ...
            <?php foreach ($services as $service): ?>
                <div class="service-item">
                    ...
                    <span class="service-price">
                        <?php 
                        // Check if this is first appointment with referral
                        $referralCheckQuery = "SELECT c.UsedReferralCode, 
                                             (SELECT COUNT(*) FROM Appointments a WHERE a.CustomerID = c.UserID) as appointment_count
                                             FROM Customers c 
                                             WHERE c.UserID = ?";
                        ...
                        if ($referralData['UsedReferralCode'] && $referralData['appointment_count'] == 0) {
                            ...
                            echo '<span class="original-price">BDT ' . number_format($originalPrice, 2) . '</span>';
                            echo '<span class="discounted-price">BDT ' . number_format($discountedPrice, 2) . '</span>';
                            echo '<span class="discount-badge">10% OFF</span>';
                        } else {
                            echo 'BDT ' . number_format($service['price'], 2);
                        }
                        ?>
                    </span>
                </div>
        ...
        <div class="total-amount">
            <strong class="paynumber"> Pay to : 01717171717</strong>
            <strong class="total-amount">Total Amount:</strong> 
            <?php 
            $totalAmount = 0;
            foreach ($services as $service) {
                if ($referralData['UsedReferralCode'] && $referralData['appointment_count'] == 0) {
                    $totalAmount += $service['price'] * 0.9;
                } else {
                    $totalAmount += $service['price'];
                }
            }
            echo 'BDT ' . number_format($totalAmount, 2);
            if ($referralData['UsedReferralCode'] && $referralData['appointment_count'] == 0) {
                echo ' <span class="discount-badge">(10% OFF Applied)</span>';
            }
            ?>
        </div>
    </div>
